...
import { 
  ...
  selectTagsBySection,
  selectTagsToDelete } from '../redux/tagSlice';
import Tag from './Tag';
...
const TagArea: React.FC<{ sectionId: number }> = ({ sectionId }) => {
  ...
  const allTags = useAppSelector(state => selectTagsBySection(state, sectionId))
  const { 
    isAndFilter, 
    activeTagIds, 
    activeNoteIds } = useAppSelector(state => selectTagMetaBySection(state, sectionId))
      ...
      <TagAreaContainer>
        ...
        <TagsViewContainer>
          {allTags
            ...
            .filter(tag =>
              activeTagIds.includes(tag.id) ? false :
              activeTagIds.length === 0 || !isAndFilter ? true :
        ...
        <TagsViewContainer>
          {activeTagIds.map((tagId: number) => <Tag key={tagId} tagId={tagId} />)}
        </TagsViewContainer>
      </TagAreaContainer>}
    </div>
  );
